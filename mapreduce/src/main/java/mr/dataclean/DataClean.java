...
public class DataClean {
     ...
     * LongWritable 偏移量, Text 块的一行数据, Text 输出的key（块的一行数据作为key）,
     * NullWritable 因为不需要计数，只是清洗，所以输出的value为NullWritable
     */
    public static class DataCleanMapper extends Mapper<LongWritable, Text, Text, NullWritable> {
        ...
        static NullWritable nullValue = NullWritable.get();

        @Override
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            /**
             * 自定义计数器，用于记录残缺记录数，实际工作中应保存在某个文件中，
             * 进行持久化
             */
